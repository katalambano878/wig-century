...
export default function CartPage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ── HERO BANNER ──────────────────────────────────── */}
      <section className="relative bg-stone-950 overflow-hidden" style={{ minHeight: '38vh' }}>
        ...
        >
          <div className="flex items-center gap-4 mb-6">
            ...
            <span className="text-amber-400 text-[9px] font-black tracking-[0.55em] uppercase">Your Bag</span>
          </div>
        ...
        {cartItems.length === 0 && savedItems.length === 0 ? (
          ...
          <div className="min-h-[50vh] flex flex-col items-center justify-center text-center py-24">
            <div className="w-20 h-20 rounded-2xl bg-stone-50 border border-stone-100 flex items-center justify-center mb-6">
              <i className="ri-shopping-cart-line text-3xl text-stone-300" />
            </div>
            <h2 className="font-serif text-3xl italic text-stone-900 mb-3">Your cart is empty</h2>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex items-center justify-between pb-5 border-b border-stone-100">
                <div>
                  <p className="text-[9px] font-black tracking-[0.5em] uppercase text-stone-300 mb-1">Review</p>
                  <h2 className="font-serif text-2xl italic text-stone-900">
                    Cart Items <span className="text-stone-300 font-light">({cartItems.length})</span>
                  </h2>
                </div>
                {savings > 0 && (
                ...
                {cartItems.map((item) => (
                  <div key={`${item.id}-${item.variant || ''}`} className="flex flex-col sm:flex-row gap-5 py-7 group">
                    ...
                    <Link
                      ...
                      className="relative w-full sm:w-28 h-40 sm:h-28 flex-shrink-0 bg-stone-50 overflow-hidden rounded-xl"
                    >
                      <Image
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="flex-1 flex flex-col justify-between min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <Link
                            href={`/product/${item.slug || item.id}`}
                            className="font-semibold text-stone-900 hover:text-stone-600 transition-colors line-clamp-2 text-base leading-snug"
                          >
                            {item.name}
                          </Link>
                          <div className="flex items-center gap-3 mt-1.5">
                            {item.variant && (
                              <span className="text-[10px] font-bold tracking-wider uppercase text-stone-400 bg-stone-50 px-2 py-0.5 rounded-full">
                                {item.variant}
                              </span>
                            )}
                        ...
                        <button
                          ...
                          className="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-lg text-stone-300 hover:text-red-500 hover:bg-red-50 transition-all"
                        >
                      ...
                      <div className="flex items-center justify-between flex-wrap gap-4 mt-4">
                        <span className="text-xl font-black text-stone-900">GH₵{item.price.toFixed(2)}</span>

                        <div className="flex flex-col items-end gap-1">
                          <div className="flex items-center border border-stone-200 rounded-xl overflow-hidden">
                            <button
                              onClick={() => updateQuantity(item.id, item.quantity - 1, item.variant)}
                              className="w-9 h-9 flex items-center justify-center text-stone-500 hover:bg-stone-50 transition-colors cursor-pointer"
                            ...
                            <input
                              ...
                              className="w-10 h-9 text-center text-sm font-bold text-stone-900 border-x border-stone-200 focus:outline-none bg-white"
                              min={item.moq || 1}
                              max={item.maxStock}
                            />
                            <button
              ...
              {savedItems.length > 0 && (
                <div className="pt-6 border-t border-stone-100">
                  <p className="text-[9px] font-black tracking-[0.5em] uppercase text-stone-300 mb-5">Saved for Later</p>
                  <div className="divide-y divide-stone-100">
                    {savedItems.map((item) => (
                      <div key={item.id} className="flex gap-4 py-5">
                        <div className="relative w-16 h-16 flex-shrink-0 bg-stone-50 rounded-xl overflow-hidden">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-base font-black text-stone-900 mt-0.5">GH₵{item.price.toFixed(2)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {/* ── ORDER SUMMARY ──────────────────────────── */}
            <div>
              <div className="bg-stone-950 rounded-2xl p-7 sticky top-24 relative overflow-hidden">
                ...
                <p className="text-[9px] font-black tracking-[0.5em] uppercase text-stone-500 mb-2">Breakdown</p>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  {appliedCoupon && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-amber-400">Coupon ({appliedCoupon.code})</span>
                      <span className="text-sm font-semibold text-amber-400">−GH₵{couponDiscount.toFixed(2)}</span>
                    </div>
                  )}

                  <div className="flex justify-between items-center">
                    <span className="text-sm text-stone-400">Shipping</span>
                ...
                <div className="border-t border-stone-800 pt-5 mb-6">
                  <div className="flex justify-between items-center">
                ...
                <Link
                  href="/checkout"
                  className="flex items-center justify-center gap-3 w-full bg-white hover:bg-amber-400 text-stone-900 py-4 rounded-xl font-bold text-xs tracking-[0.25em] uppercase transition-colors mt-6 mb-3"
                >
                ...
                <Link
                  href="/shop"
                  className="flex items-center justify-center gap-2 w-full text-center text-stone-500 hover:text-white text-xs font-bold tracking-[0.2em] uppercase py-2 transition-colors"
                >
                  <i className="ri-arrow-left-line" /> Continue Shopping
                </Link>

                {/* Trust signals */}
                <div className="mt-7 pt-6 border-t border-stone-800 grid grid-cols-3 gap-3">
                  {[
                    ...
                    <div key={t.label} className="flex flex-col items-center gap-1.5 text-center">
                      <i className={`${t.icon} text-amber-400 text-lg`} />
                      <span className="text-[8px] font-bold tracking-wider uppercase text-stone-600 leading-tight">{t.label}</span>
                    </div>
                  ))}
                </div>
              </div>
